...
const adminNavItems: NavItem[] = [
    ...
    },
    {
        ...
        roles: ["Admin", "Employee"],
        subItems: [
            { name: "Time Tracking", path: "/attendance/time", roles: ["Admin", "Employee"] },
            { name: "Leave Management", path: "/attendance/leave", roles: ["Admin", "Employee"] },
            { name: "Attendance Reports", path: "/attendance/reports", roles: ["Admin"] },
        ],
    },
    {
        icon: <DollarSignIcon />,
...
const AppSidebar: React.FC = () => {
    ...
    const mainNavItems = filterNavItemsByRole(userRole === "Admin" ? adminNavItems : employeeNavItems, userRole);
    const othersNavItems = filterNavItemsByRole(userRole === "Admin" ? adminOthersItems : employeeOthersItems, userRole);

    const handleToggle = (index: number, type: "main" | "others") => {
        setOpenSubmenu((prev) => (prev?.index === index && prev.type === type ? null : { type, index }));
    };
                    ...
                    {nav.subItems ? (
                        <>
                            <button
                                ...
                                className={`menu-item group w-full cursor-pointer flex items-center gap-3 p-3 rounded-lg transition-all duration-200 ${openSubmenu?.type === type && openSubmenu.index === index ? "menu-item-active" : "menu-item-inactive"
                                    } ${!isExpansionState ? "justify-center" : "justify-start"}`}
                            >
                                <span className="flex-shrink-0 w-5 h-5 flex items-center justify-center">{nav.icon}</span>
                                {isExpansionState && (
                                    <>
                                        <span className="flex-1 truncate">{nav.name}</span>
                                        <ChevronDownIcon
                                            className={`w-4 h-4 transition-transform duration-200 ${openSubmenu?.type === type && openSubmenu.index === index ? "rotate-180 text-brand-500" : ""
                                                }`}
                                        />
                                    </>
                                )}
                            ...
                            {/* Dropdown */}
                            <div
                                className={`overflow-hidden transition-all duration-300 ${openSubmenu?.type === type && openSubmenu.index === index ? "max-h-96" : "max-h-0"
                                    }`}
                            >
                                <ul className="mt-2 space-y-1 pl-8">
                                    {nav.subItems.map((sub) => (
                                        <li key={sub.name}>
                                            <Link
                                                href={sub.path}
                                                className={`menu-dropdown-item w-full flex items-center justify-between p-2 rounded-md transition-all duration-200 ${isActive(sub.path) ? "menu-dropdown-item-active" : "menu-dropdown-item-inactive"
                                                    }`}
                                            >
                        ...
                        nav.path && (
                            <Link
                                href={nav.path}
                                className={`menu-item group w-full flex items-center gap-3 p-3 rounded-lg transition-all duration-200 ${isActive(nav.path) ? "menu-item-active" : "menu-item-inactive"
                                    } ${!isExpansionState ? "justify-center" : "justify-start"}`}
                            >
                                <span className="flex-shrink-0 w-5 h-5 flex items-center justify-center">{nav.icon}</span>
                                {isExpansionState && <span className="flex-1 truncate">{nav.name}</span>}
                            </Link>
                        )
                    )}
                </li>
    ...
    return (
        <aside
            className={`fixed mt-16 top-0 left-0 h-screen bg-white dark:bg-[#0A0A0A] dark:text-gray-50 border-r border-gray-200 dark:border-[#121212] transition-all duration-300 z-50 ${isExpansionState ? "w-[290px]" : "w-[90px]"
                }`}
            onMouseEnter={() => !isExpanded && setIsHovered(true)}
            onMouseLeave={() => setIsHovered(false)}
        >
            {/* Logo */}
            <div className={`py-6 px-4 flex items-center ${!isExpansionState ? "justify-center" : "justify-start"}`}>
                <Link href="/">
                    {isExpansionState ? (
                        <>
            ...
            <div className="flex-1 flex flex-col overflow-y-auto px-4 pb-4">
                <nav className="flex-1 flex flex-col gap-6">
                    {/* Main Menu */}
                    <div>
                        <h2 className={`mb-3 text-xs uppercase font-semibold text-gray-400 flex items-center ${!isExpansionState ? "justify-center" : "justify-start"}`}>
